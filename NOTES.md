...
- The form can find existing invoices through the read-only `GET /api/invoices/?search=...&currency=EUR&status=open` endpoint, with the same pagination, or retrieve one with `GET /api/invoices/<id>/`. These expose local ID, vendor ID, customer name, currency, and status, and do not permit invoice writes.
